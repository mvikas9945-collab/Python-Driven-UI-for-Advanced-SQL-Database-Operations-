# interface file
# create a library in terminal pip install mysql-connector-python
# create steamlit library by using we gonna create interface pip install

import streamlit as st
import pandas as pd

# then using streamlit documentation in google we utilize that because they already bulit some wriiten code
# in that documentation they show functions how they works
# callin functions from db_function file to interface file
...
if option=="Basic Information":
    ...
    # display like columns

    cols=st.columns(3)
    keys=list(basic_info.keys())# printing like only value

    for i in range(3):#picking the info from dict by using label
    ...
    st.divider() # it prints one divider line
    # fetch and display details in tables
    ...
    selected_task=st.selectbox("Choose an option",["Add New Product","Product History","Place Reorder","Received Reorder"])
if selected_task =="Add New Product":
        st.header("Add New Product")
        categories=get_categories(cursor)
        suppliers= get_suppliers(cursor)
# this is for icons interface
